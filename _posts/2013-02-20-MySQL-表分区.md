...
# 3. 建表并分区(这里使用RANGE方式):
	注: 不论何种类型的分区，如果表中存在primary key或者unique key时，分区列必须是unique key的一个组成部分。
	unique key允许NULL，并且分区只要是唯一索引的一个组成部分，并不要求是整个unique key都是分区列。
	当建表时没有指定primary key或unique key时，可以指定任何一个列为分区列。
************************************************
